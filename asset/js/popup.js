...
function closeBottomSheet() {
	...
	document.querySelector('.bottom-sheet-wrap.open').classList.remove('open');
	scrollAble();
	sheetOpen = false;
...
function startDragging(e) {
	...
	$('.bottom-sheet-wrap.open .sheet-stretch').css('transition', 'none');
...
function drag(e) {
	...
	$('.bottom-sheet-wrap.open .sheet-stretch').height(sheetCurrentHeight);
}
...
function stopDragging() {
	...
	$('.bottom-sheet-wrap.open .sheet-stretch').css('transition', 'height 0.3s ease-out, transform 0.3s ease-out');
	if (sheetCurrentHeight > sheetInitialHeight) {
		$('.bottom-sheet-wrap.open .sheet-stretch').height(sheetCurrentHeight);
	} else if (sheetCurrentHeight < sheetInitialHeight * 0.5) {
		closeBottomSheet();
	} else {
		$('.bottom-sheet-wrap.open .sheet-stretch').height(sheetInitialHeight);
		sheetCurrentHeight = sheetInitialHeight;
	}
}
